...
export const productShowAction = () => async (dispatch, getState) => {
  try {
    ...
    let { data } = await axios.get("/api/admin/productShow", config);
...
export const ShowAllProAction = () => async (dispatch, getState) => {
  try {
    ...
    let { data } = await axios.get("/api/user/ShowAllPro", config);
...
export const addToCartProAction = (id) => async (dispatch, getState) => {
  try {
    ...
    let { data } = await axios.get(`/api/user/addToCartPro?id=${id}`, config);

    dispatch({ type: ADD_TO_CARD_SUCCESS, payload: data });
    console.log(data, "datadatadatadatadata");
  } catch (error) {
    console.log(error.response.data, "errorerrorerrorerrorerror");
    dispatch({
      type: ADD_TO_CARD_ERR,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
